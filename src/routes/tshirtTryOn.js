...
async function getMongoCollection() {
  ...
  return db.collection("tshirtImages");
}
...
router.post("/", async (req, res) => {
  const { personUrl, tshirtUrl, color, name, number, seed } = req.body || {};
  const t0 = Date.now();

  // Log incoming request for debugging
  try {
    console.log("/api/tshirt-tryon incoming:");
    console.log("- headers:", JSON.stringify(req.headers, null, 2));
    console.log("- body:", JSON.stringify(req.body, null, 2));
  } catch (_) {}

  if (!personUrl || !tshirtUrl) {
    return res.status(400).json({
      ok: false,
      error: "personUrl and tshirtUrl are required in JSON body",
      receivedKeys: Object.keys(req.body || {}),
      hint: "Send application/json with { personUrl: string, tshirtUrl: string }"
    });
  }
  ...
  try {
    ...
    console.log("[tshirt-tryon] STEP 1: Fetching input images...");
    const [personPart, tshirtPart] = await Promise.all([
      urlToInlineDataPart(personUrl),
      urlToInlineDataPart(tshirtUrl)
    ]);

    const tFetchEnd = Date.now();
    console.log(`[tshirt-tryon] STEP 1 DONE in ${tFetchEnd - t0}ms`);

    const client = new GoogleGenAI({ apiKey });

    const promptText = `Create a single photorealistic photograph combining the two inputs.
    Input 1 is the person (face and body). Input 2 is the t-shirt${color ? `. Change the t-shirt color to ${color}` : ""}.
    
    IMPORTANT: Completely replace any existing clothing the person is wearing with the t-shirt from Input 2. 
    Whether the person is wearing a full sleeve, half sleeve, or any other type of shirt/tshirt, 
    it must be completely replaced by the t-shirt from Input 2, maintaining its exact style, 
    ...
    Place and align the t-shirt naturally on the person's body, ensuring proper fit and realistic draping.
    The t-shirt should look like it's actually being worn by the person, with natural wrinkles and shadows.
    Match lighting and color temperature between person and t-shirt; avoid artifacts or extra accessories.
    ...
    if (seed != null) {
      console.log(`[tshirt-tryon] Using seed: ${Number(seed)}`);
    }

    console.log("[tshirt-tryon] STEP 2: Sending to Gemini...");
      ...
      contents: [
        {
          ...
          parts: [
            { text: promptText },
            personPart,    // { inlineData: ... }
            tshirtPart     // { inlineData: ... }
          ]
        }
      ],
      config: {
    ...
    console.log(`[tshirt-tryon] STEP 2 DONE in ${tGenEnd - tFetchEnd}ms (cumulative ${tGenEnd - t0}ms)`);
    ...
    if (!inlineData?.data) {
      ...
      console.log("/api/tshirt-tryon: Model refused. Reason:", refusalText);
    ...
    const doc = {
      ...
      createdAt: new Date(),
      personUrl: String(personUrl || ""),
      tshirtUrl: String(tshirtUrl || ""),
      color: typeof color === "string" ? color : (color != null ? String(color) : ""),
    ...
    console.log(`[tshirt-tryon] STEP 3 DONE: Saved to Mongo in ${tInsertEnd - tGenEnd}ms (id=${insert.insertedId})`);
    ...
    console.log("[tshirt-tryon] STEP 4 DONE: URL prepared and document updated");
    console.log(`[tshirt-tryon] SUMMARY => total=${tDone - t0}ms, fetch=${tFetchEnd - t0}ms, gen=${tGenEnd - tFetchEnd}ms, db=${tInsertEnd - tGenEnd}ms, mime=${inlineData.mimeType || "image/jpeg"}, b64_len=${inlineData.data.length}, url=${url}`);
    return res.status(200).json({ ok: true, url, id: insert.insertedId.toString() });

  } catch (err) {
    const tErr = Date.now();
    console.error("/api/tshirt-tryon generation error:", err?.response?.data || err.message, `after ${tErr - t0}ms`);
    return res.status(200).json({ ok: false, url: "", id: "", error: err.message || "Generation failed" });
  }
});

// GET /api/tshirt-tryon/:id -> stream stored image
router.get("/:id", async (req, res) => {
  try {
    ...
    console.error("/api/tshirt-tryon/:id error:", e.message);
